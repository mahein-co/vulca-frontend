...
export default function LineChartROE({ globalDateStart, globalDateEnd }) {
  ...
  useEffect(() => {
    ...
    const fetchEvolutionData = async () => {
      ...
      try {
        ...
        let url = `${BASE_URL_API}/evolution-roe/`;
        ...
        if (error.name !== 'AbortError') {
          console.error('Erreur chargement évolution ROE:', error);
        }
      } finally {
        if (!abortController.signal.aborted) {
          setLoading(false);
    ...
    <div className="w-full h-80 md:h-96 relative">
      {loading ? (
        <div className="w-full h-full flex items-center justify-center">
          <div className="text-gray-500">Chargement de l'évolution du ROE...</div>
        </div>
      ) : (
