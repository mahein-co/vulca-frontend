...
export default function IndexChatbotPage({ close }) {
  ...
  const { data: histories, isLoading: loadingHistories } =
    useGetHistoriesQuery(projectId);
  ...
  const {
    ...
    { historyId: selectedHistoryId, project_id: projectId },
  ...
  useEffect(() => {
    ...
    const rawMessages =
      dbMessages?.history?.chat_messages || dbMessages?.chat_messages || [];
  ...
  // ✅ RESILIENCE: Si l'historique sélectionné n'existe plus (404), on réinitialise
  useEffect(() => {
    if (isError && error?.status === 404) {
      console.warn("L'historique sélectionné est introuvable (404). Réinitialisation...");
  ...
  const handleNewChat = async () => {
    try {
      ...
      Swal.fire({
        title: "Erreur",
        text:
          err?.data?.message || "Impossible de créer une nouvelle discussion",
        icon: "error",
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      console.log(
        "📊 Données filtrées incluses:",
        filteredData ? "OUI" : "NON",
      );
      if (filteredData)
        console.log("🔍 Détail données filtrées:", filteredData);
      ...
      if (response?.suggested_filter) {
        ...
        const { type, value, label } =
          response.suggest_filter || response.suggested_filter;
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto space-y-1.5 custom-scrollbar pr-1">
          {loadingHistories ? (
            <div className="opacity-40 text-sm animate-pulse text-center">
              Chargement...
            </div>
          ) : Array.isArray(histories?.histories || histories) &&
            ...
            (histories?.histories || histories).map((h) => (
              ...
              >
                ...
                {!editingHistoryId && (
                  <div
                    className="flex gap-2 ms-auto"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <button
                      onClick={() => handleRename(h.id, h.title)}
            ...
            <div className="text-[10px] opacity-20 text-center py-4">
              Aucune discussion
            </div>
          )}
        </div>
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden z-10 w-full">
        {/* HEADER */}
        <header
          className="px-5 py-4 border-b shrink-0 flex items-center justify-between z-[80]"
        ...
        >
          <div className="flex items-center gap-4">
            <button
              ...
              title={
                isSidebarOpen ? "Masquer l'historique" : "Afficher l'historique"
              }
            >
              <FaBars size={18} />
            </button>
        ...
        >
            ...
            messages.map((m, i) => (
              ...
              >
                ...
                >
                  ...
                  {m.calculation && (
                    <div className="mt-3">
                      {typeof m.calculation === "string" &&
                      (m.calculation.endsWith(".xlsx") ||
                        m.calculation.endsWith(".pdf") ||
                        m.calculation.endsWith(".xls")) ? (
                        <div
                          className={`flex items-center gap-4 p-3 rounded-xl border transition-all cursor-pointer hover:shadow-md ${
                            isDarkMode
                        ...
                        >
                          <div
                            className={`p-3 rounded-lg flex-shrink-0 ${isDarkMode ? "bg-slate-800" : "bg-slate-700"} text-white shadow-inner`}
                          >
                            {m.calculation.endsWith(".xlsx") ||
                            m.calculation.endsWith(".xls") ? (
                              <FaFileExcel
                        ...
                        (!Array.isArray(m.calculation) ||
                          m.calculation.length <= 1) && (
                          <div
                            className={`p-3 rounded-xl ${isDarkMode ? "bg-indigo-500/10 border border-indigo-500/20" : "bg-indigo-50 border border-indigo-100"}`}
                          >
                            <p className="text-[8px] uppercase font-bold tracking-widest opacity-60 mb-1">
                              Résultat
                            </p>
                            <p
                              className="text-lg font-mono font-black italic tracking-tighter leading-none"
                              style={{ color: textColor }}
                            >
                              {typeof m.calculation === "object"
                                ? JSON.stringify(m.calculation)
                                : String(m.calculation)}
                              {typeof m.calculation === "number" && (
                                <span className="text-[10px] ml-1 not-italic font-sans opacity-70">
                                  AR
                                </span>
                              )}
                            </p>
          ...
          {sendingMessage && (
            <div className="flex justify-start">
              <div
                className={`p-4 rounded-2xl rounded-tl-none border ${isDarkMode ? "bg-gray-800 border-gray-700/50" : "bg-white border-gray-200"}`}
              >
      ...
      <style
        ...
        /* Ensure SweetAlert2 is above the Chatbot container */
        .swal2-container.z-\\[10020\\] {
